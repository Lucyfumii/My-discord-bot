...
client.once('ready', async function() {
    console.log(`[HUOHUO_CONFIG] Loading Configuration... (Config Version: ${process.env.CFG_VERSION || 'N/A'})`)
    const rest = new REST({ version: "10" }).setToken(process.env.TOKEN);

    try {
        await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
        console.log("[HUOHUO_CMDS] Commands registered (production)!");
    } catch (err) {
        console.error(err);
    }
})
...
if (!process.env.TOKEN) {
    console.log(`[HUOHUO_ERROR] The .env file could not be found/doesn't exist.`)
    process.exit(0)
}

client.login(process.env.TOKEN)
.catch((err) => {
    console.log(`[HUOHUO_ERROR] Bot could not login and authenticate with Discord. Have you populated your .env file with your bot token and copied it over correctly? (Using token: ${process.env.TOKEN})\nError Trace: ${err}`);
})